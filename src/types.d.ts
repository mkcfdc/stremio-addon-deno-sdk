...
export interface Args {
    ...
    extra: Record<string, unknown>; // Make extra more flexible
...
export class AddonBuilder { // Capitalized class name
...
export interface Manifest {
    ...
    /**
     * Secret key for encrypting/decrypting user configuration via JWE (A256GCM).
     * If the 'config' array is defined, this property is REQUIRED.
     * Configuration in the URL will be JWE encrypted then base64 encoded.
     * IMPORTANT: This secret MUST NOT be included in the served manifest.json. Keep it secure.
     */
    encryptionSecret?: string; // Renamed and potentially required if config exists
...
export interface ManifestConfig {
    ...
    title: string; // Make title required for landingTemplate compatibility
    ...
    options?: string[]; // Correct type to string array
    ...
    required?: boolean; // Correct type to boolean
...
export function getRouter(addonInterface: AddonInterface): (request: Request) => Promise<Response>; // More specific return type
...
export function publishToCentral(url: string): Promise<any>; // Corrected return type
...
): Promise<{ url: string; close: () => void }>; // Corrected return type
